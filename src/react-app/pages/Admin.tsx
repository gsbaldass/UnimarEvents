...
import { Calendar, Clock, MapPin, Users, CheckCircle, XCircle, Hourglass, ArrowLeft, Mail, Shield, LogOut } from "lucide-react";
import Header from "@/react-app/components/Header";
import { getAllBookings, getVenues, updateBookingStatus } from "@/react-app/services/firestoreDatabase";
import type { FirestoreBooking } from "@/react-app/services/firestoreDatabase";

interface AdminBooking extends FirestoreBooking {
  venue_name: string;
...
export default function Admin() {
  ...
  const handleAction = async () => {
    ...
    try {
      ...
      const rejectionReasonValue = actionType === "reject" ? rejectionReason : undefined;
  ...
  const openActionModal = (booking: AdminBooking, action: "approve" | "reject") => {
    ...
    <div className="min-h-screen bg-white">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
            ...
            <div className="flex items-center space-x-4 mb-4">
              ...
              <div>
                ...
                <p className="text-blue-100 mt-1">
                  Sistema de gestão de agendamentos de eventos
                </p>
              </div>
            </div>
          </div>
        ...
        <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-6 mb-8">
          <h3 className="text-lg font-semibold text-gray-900 mb-4">Filtrar Solicitações</h3>
          <div className="flex flex-wrap gap-3">
            {[
              { key: "all", label: "Todas as Solicitações", count: bookings.length },
        ...
        {filteredBookings.length === 0 ? (
          <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-12 text-center">
            ...
            <p className="text-gray-600">
              {filter === "all" 
                ? "Não há solicitações de agendamento no sistema"
                : `Não há solicitações ${filter === "pending" ? "pendentes" : filter === "approved" ? "aprovadas" : "rejeitadas"}`
              }
            </p>
          </div>
        ) : (
          ...
          <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50">
            <div className="bg-white rounded-lg max-w-md w-full p-6">
              <h3 className="text-lg font-semibold text-gray-900 mb-4">
                {actionType === "approve" ? "Aprovar" : "Rejeitar"} Agendamento
              </h3>
              
              <div className="mb-4">
                <p className="text-sm text-gray-600 mb-2">
                  <strong>Evento:</strong> {selectedBooking.event_title}
                </p>
                <p className="text-sm text-gray-600 mb-2">
                  <strong>Empresa:</strong> {selectedBooking.company_name}
                </p>
                <p className="text-sm text-gray-600">
                  <strong>Data:</strong> {formatDate(selectedBooking.event_date)} às {formatTime(selectedBooking.start_time)}
                </p>
              </div>
              ...
              <div className="flex justify-end space-x-3">
                ...
                <button
                  ...
                  className={`px-4 py-2 text-white rounded-lg font-medium transition-colors disabled:bg-gray-400 disabled:cursor-not-allowed ${
                    actionType === "approve" 
                      ? "bg-green-600 hover:bg-green-700" 
                      : "bg-red-600 hover:bg-red-700"
                  }`}
                >
                  {submitting ? "Processando..." : actionType === "approve" ? "Aprovar" : "Rejeitar"}
                </button>
              </div>
            </div>
          </div>
